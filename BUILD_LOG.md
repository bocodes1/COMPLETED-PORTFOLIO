...
- **Reactive pass (2026-06-11)**: user wanted the page to feel less static without changing design/layout. Added: interactive EquityChart (draw-in on reveal, hover crosshair + per-window readout, breathing endpoint), TickStream (rolling latency bars, ~700ms updates after mount, deterministic first frame for SSR), StatCounter (count-up telemetry on reveal), scrollspy nav (active section underlined). All grayscale, all gated behind prefers-reduced-motion checks. Illustrative labels stay on both charts.
